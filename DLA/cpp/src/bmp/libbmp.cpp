...
void BmpPixbuf::init(const int width,
					 const int height)
{
...
void BmpPixbuf::set_pixel(const int x,
						  const int y,
...
void BmpPixbuf::write(const int row,
					  std::ofstream &f)
{
	f.write(reinterpret_cast<char *>(&data[row * len_row]), len_row);
}

void BmpPixbuf::read(const int row,
					 std::ifstream &f)
{
			   ...
			   const int height) : BmpPixbuf(width, std::abs(height))
{
	...
	header.bfSize = (3 * width + BMP_GET_PADDING(width)) * std::abs(height);
...
int BmpImg::get_width(void)
{
	return header.biWidth;
}

int BmpImg::get_height(void)
{
